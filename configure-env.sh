#!/bin/bash
# =============================================================================
# CRM System Environment Configuration Script
# =============================================================================
# Run this script in your Codespace to configure all environment variables
#
# Usage: bash configure-env.sh <GOOGLE_CLIENT_SECRET>
# Example: bash configure-env.sh "GOCSPX-xxxxx"

set -e

GOOGLE_CLIENT_SECRET="${1}"

if [ -z "$GOOGLE_CLIENT_SECRET" ]; then
    echo "❌ Error: Google Client Secret is required"
    echo "Usage: bash configure-env.sh <GOOGLE_CLIENT_SECRET>"
    echo ""
    echo "To get your client secret:"
    echo "1. Open the downloaded JSON file"
    echo "2. Find the 'client_secret' value"
    echo "3. Run: bash configure-env.sh \"YOUR_CLIENT_SECRET\""
    exit 1
fi

echo "🔧 Configuring CRM System Environment..."

# Create .env file
cat > .env << 'EOF'
# =============================================================================
# CRM System Environment Variables - AUTO-CONFIGURED
# =============================================================================

# Database Configuration
DATABASE_URL=postgresql+asyncpg://crm_user:password@db:5432/crm_db
POSTGRES_USER=crm_user
POSTGRES_PASSWORD=password
POSTGRES_DB=crm_db

# Redis Configuration
REDIS_URL=redis://redis:6379/0

# Elasticsearch Configuration
ELASTICSEARCH_URL=http://elasticsearch:9200

# JWT Configuration
JWT_SECRET_KEY=crm-production-secret-key-2026-change-this-in-real-production
JWT_ALGORITHM=HS256
JWT_ACCESS_TOKEN_EXPIRE_MINUTES=60
JWT_REFRESH_TOKEN_EXPIRE_DAYS=30

# OAuth 2.0 - Google/Gmail
GOOGLE_CLIENT_ID=59109027316-5ceod4udv4s2c1s16atvnlr7dko3dl14.apps.googleusercontent.com
EOF

# Add the client secret from command line argument
echo "GOOGLE_CLIENT_SECRET=${GOOGLE_CLIENT_SECRET}" >> .env

cat >> .env << 'EOF'
GOOGLE_REDIRECT_URI=https://urban-broccoli-r4q5ggx57wr52474-8000.app.github.dev/api/v1/auth/oauth/gmail/callback

# Gmail API Scopes
GMAIL_SCOPES=https://www.googleapis.com/auth/gmail.readonly,https://www.googleapis.com/auth/gmail.send,https://www.googleapis.com/auth/gmail.modify

# Encryption Key
ENCRYPTION_KEY=KMYSYbArdxhE7ln3ReQLtLYSX0DVmjkrsqGwGdgYJLQ=

# AI Service Configuration
AI_PROVIDER=gemini
GEMINI_API_KEY=AIzaSyA4UBH18aZehgXkgYpr8sgWbCIDIDPkp-w
GEMINI_MODEL=gemini-pro

# Email Configuration
SMTP_HOST=mailhog
SMTP_PORT=1025
SMTP_FROM_EMAIL=noreply@crm.local

# Storage
STORAGE_BACKEND=local
STORAGE_LOCAL_PATH=/app/storage

# Pub/Sub
PUBSUB_BACKEND=redis

# Rate Limiting
RATE_LIMIT_PER_MINUTE=100

# CORS - Codespace URLs
CORS_ORIGINS=https://urban-broccoli-r4q5ggx57wr52474-3000.app.github.dev,https://urban-broccoli-r4q5ggx57wr52474-8000.app.github.dev,http://localhost:3000,http://localhost:8000

# Frontend
NEXT_PUBLIC_API_URL=https://urban-broccoli-r4q5ggx57wr52474-8000.app.github.dev
NEXT_PUBLIC_WS_URL=wss://urban-broccoli-r4q5ggx57wr52474-8000.app.github.dev

# Logging
LOG_LEVEL=INFO
ENVIRONMENT=development
DEBUG=true
EOF

echo "✅ .env file created successfully!"
echo ""
echo "📋 Configuration Summary:"
echo "  ✓ Google Client ID: 59109027316-5ceod4udv4s2c1s16atvnlr7dko3dl14.apps.googleusercontent.com"
echo "  ✓ Google Client Secret: ${GOOGLE_CLIENT_SECRET:0:10}..."
echo "  ✓ Gemini API Key: AIzaSyA4UBH18aZehgXkgYpr8sgWbCIDIDPkp-w"
echo "  ✓ Encryption Key: Generated"
echo "  ✓ Codespace URLs: Configured"
echo ""
echo "🚀 Next steps:"
echo "  1. Restart services: docker-compose -f docker-compose.dev.yml restart"
echo "  2. Check logs: docker-compose -f docker-compose.dev.yml logs -f auth ai"
echo "  3. Test OAuth: Visit https://urban-broccoli-r4q5ggx57wr52474-8000.app.github.dev/docs"
echo ""
echo "✨ Configuration complete!"
